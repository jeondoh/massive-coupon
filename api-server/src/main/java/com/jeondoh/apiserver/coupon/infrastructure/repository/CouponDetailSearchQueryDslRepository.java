...
import com.jeondoh.apiserver.core.util.QuerydslHelper;
...
public class CouponDetailSearchQueryDslRepository {
    ...
    // 이벤트 쿠폰 검색
    public Page<SearchEventCouponResponse> findByEventCoupon(Pageable pageable, Sort sort, String memberId) {
        // 카운트 쿼리
        Long totalSize = queryFactory
                .select(couponDetail.count())
                .from(couponDetail)
                ...
                .on(coupon.couponDetail.id.eq(couponDetail.id)
                        .and(coupon.memberId.eq(Long.parseLong(memberId))))
